...
public class TransactionRecordUtility {
	...
	//We need to make private back
	public TransactionRecordUtility() {}
	...
	public TransactionRecord setTransactionRecordThroughReflection(String fieldName,String fieldValue, TransactionRecord transactionRecord  ) {
		

		Class transactionRecordClass = transactionRecord.getClass();
		
		try {
			//We are getting all the fields of TransactionRecord class.
			Field field = transactionRecordClass.getDeclaredField(fieldName);
			field.setAccessible(true);
			if(field.getType().equals(String.class)) {
				//Set the value "fieldValue.toString()" to 'field' of TransactionRecord class
				field.set(transactionRecord, fieldValue.toString());
			}
		} catch (Exception e) {
			e.printStackTrace();
		} 
