 ...
 */
public class ReadFromDynamoDB_Pagination_Validation2 implements RequestHandler<Object, Parameters> {
	...
	public Parameters handleRequest(Object arg0, Context arg1) {
		...
		do {
			...
			String updatedNewValue = "Validation2";
			QueryResult result = client.query(queryRequest);
			for (Map<String, AttributeValue> item : result.getItems()) {
				System.out.println("item: " + item);
				if (item.get("specification_category").getS().equalsIgnoreCase("car")) {
					/** item.get("Id").getS() : to get the exact PK value **/
					UpdateDynamoDbTable.updateColumnValueBasedOnPK(item.get("Id").getS(), updatedNewValue, "7");
				}
			}
