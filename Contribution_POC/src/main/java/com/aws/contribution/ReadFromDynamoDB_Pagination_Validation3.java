 ...
 */
public class ReadFromDynamoDB_Pagination_Validation3 implements RequestHandler<Object, Parameters> {
	...
	@Override
	public Parameters handleRequest(Object arg0, Context arg1) {
		...
		do {
			...
			queryRequest.setIndexName("RECORD_STATUS_ID-index");// RECORD_STATUS_ID-index -> index name of DB

			String updatedNewValue = "Validation3";
			QueryResult result = client.query(queryRequest);
			for (Map<String, AttributeValue> item : result.getItems()) {
				System.out.println("item: " + item);

				if (item.get("specification_category").getS().equalsIgnoreCase("train")) {
					/** item.get("Id").getS() : to get the exact PK value **/
					UpdateDynamoDbTable.updateColumnValueBasedOnPK(item.get("Id").getS(), updatedNewValue, "55");
				}
			}
